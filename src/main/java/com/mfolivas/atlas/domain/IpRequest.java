...
public final class IpRequest {
    ...
    private IpRequest(String ip) {
        this.ip = Optional.ofNullable(ip)
                .filter(ipAddress -> ipAddress.matches(IP_REGEX))
                .orElseThrow(() -> new InvalidIpAddress(
                        MessageFormat.format("The following ip is not valid [{0}]", ip)));
    }

    public static IpRequest valueOf(final String ip) {
    ...
    public boolean equals(Object o) {
        ...
        IpRequest ipRequest = (IpRequest) o;
        return Objects.equals(ip, ipRequest.ip);
    }
